...
function TopHeadlines() {
  const params = useParams();  // Get the category from URL parameters
  ...
  function handlePrev() {
    setPage(prevPage => Math.max(prevPage - 1, 1));  // Prevent negative page numbers
  }
  ...
  useEffect(() => {
    ...
    const categoryParam = params.category ? `&category=${params.category}` : "";  // Add category if available
    ...
    fetch(url)
      ...
      .then((json) => {
        if (json.success) {
          ...
          setData(json.data.articles);  // Store articles data in state
        } else {
  ...
  }, [page, params.category]);  // Re-fetch data when page or category changes
